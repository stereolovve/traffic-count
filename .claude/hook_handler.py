...
SOUND_MAP = {
    ...
    "Stop": "radio_beep",      # Task completed - BEEP FOR TASK COMPLETION!
    "SubagentStop": "radio_beep",   # Subtask completed - BEEP FOR SUBTASK COMPLETION!
    ...
    # Task management - also trigger beep when todo is marked complete
    "TodoWrite": "airplane-beep-sound-effect",       # Update todo list with beep
...
def play_sound(sound_name):
    """
    Play a sound file using platform-specific audio commands.
    ...
    sounds_dir = script_dir / "hooks" / SOUNDS_TYPE  # Changed from "sounds" to "hooks"
    ...
    for extension in ['.wav', '.mp3']:
        ...
        if file_path.exists():
            try:
                ...
                if system == "windows":
                    # Windows: Use winsound for reliable beeping
                    import winsound
                    try:
                        # Different beeps for different sounds
                        if sound_name == "radio_beep":
                            winsound.Beep(800, 300)  # High beep for task completion
                        elif sound_name == "airplane-beep-sound-effect":  
                            winsound.Beep(600, 200)  # Lower beep for todo updates
                        else:
                            winsound.Beep(400, 150)  # Default beep
                        return True
                    except Exception:
                        # Fallback: try system notify sound
                        try:
                    ...
                    cmd = ["paplay", str(file_path)] if extension == '.wav' else ["mpg123", str(file_path)]
                else:
